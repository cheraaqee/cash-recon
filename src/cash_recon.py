...
def load_range_rows(
    ...
    week_start_date, _ = get_week_bounds(start_date)

    initial_previous_till = get_previous_till_for_day(
    ...
    return build_range_rows(
        ...
        initial_previous_till=initial_previous_till,
    )
    ...
    for row in range_rows:
        if row["has_data"]:
            print(
                ...
                f"{fmt_money(row['cash_added_to_till']):>10} "
                f"{fmt_money(row['expenses_total']):>10} "
                f"{fmt_money(row['reconciled_total']):>10} "
                f"{fmt_money(row['difference']):>10} "
                f"{fmt_money(row['cum_cash_in_report']):>10} "
                f"{fmt_money(row['cum_cash_added_to_till']):>10} "
                f"{fmt_money(row['cum_expenses_total']):>10} "
                f"{fmt_money(row['cum_reconciled_total']):>10} "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Cash reconciliation tool")
    parser.add_argument(
        "--db-path",
    ...
    add_day_cmd.add_argument(
        ...
        required=True,
        type=non_negative_decimal,
        help="End-of-day cash physically in till",
    )
    add_day_cmd.add_argument(
        "--expenses-file",
        required=True,
...
def main() -> None:
    ...
    if args.command == "add-day":
        ...
        week_start_date, _ = get_week_bounds(report_date)
        previous_till = get_previous_till_for_day(
        ...
        print(f"Previous till:        £{summary['previous_till']}")
        print(f"End-of-day till:      £{args.cash_till}")
        print(f"Cash added to till:   £{summary['cash_added_to_till']}")
        print(f"Expenses total:       £{summary['expenses_total']}")
        print(f"Reconciled total:     £{summary['reconciled_total']}")
    ...
    if args.command == "show-day":
        ...
        print(f"Previous till:        £{summary['previous_till']}")
        print(f"End-of-day till:      £{Decimal(day_row['cash_in_till'])}")
        print(f"Cash added to till:   £{summary['cash_added_to_till']}")
        print(f"Expenses total:       £{summary['expenses_total']}")
        print(f"Reconciled total:     £{summary['reconciled_total']}")
        print(f"Difference:           £{summary['difference']}")
